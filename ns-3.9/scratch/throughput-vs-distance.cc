...
void Progress ()
{
    ...
    if (sink) {
        ...
        double curTp = cur / now.GetSeconds();
        //cout << "\t 1:" << cur << " [+" << cur-last << "]" << endl;
        cout << now.GetSeconds() << " " << cur << " " << curTp << endl;
        last = cur;
    }
...
int main (int argc, char *argv[])
{
  ...
  enum WifiPhyStandard standard = WIFI_PHY_STANDARD_80211ad_OFDM;
  ...
  cmd.AddValue ("antenna", "type of the antennas: 0 is cone, 1 is measured", antenna);
  cmd.Parse (argc, argv);

  loc[1][1] += distanceToRx;
  ...
  if (standard == WIFI_PHY_STANDARD_80211b) {
    ...
    errorModel = "ns3::NistErrorRateModel";
  } else if (standard == WIFI_PHY_STANDARD_80211ad_OFDM) {
    freq = 59.4e9;
    phyMode = "VHTMCS13a";
  ...
  /* various defaults */
    ...
    Config::SetDefault ("ns3::YansWifiPhy::ExtendedRateMode", UintegerValue(0));
    ...
    Config::SetDefault ("ns3::OnOffApplication::DataRate", DataRateValue(DataRate("5Gbps"))); 
    ...
    Config::SetDefault ("ns3::UdpSocket::RcvBufSize", UintegerValue(2e6));
  ...
  /* Give all nodes Measured antennas of specified dBi gain */
  if (antenna == 0) {
        wifiPhy.SetAntenna ("ns3::ConeAntenna", "Beamwidth", DoubleValue(ConeAntenna::GainDbiToBeamwidth(dbiGain)));
  }
  else if (antenna == 1) {
      wifiPhy.SetAntenna ("ns3::Measured2DAntenna", "Mode", DoubleValue(dbiGain));
  }
  else {
      printf("bad antenna type\n");
      exit(-1);
  }
