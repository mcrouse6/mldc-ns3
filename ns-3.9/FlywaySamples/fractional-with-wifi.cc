...
namespace{
        void ChangeRoute (int)
        {
            ...
            Ipv4Address nextHop[2]; 
            nextHop[0] = Ipv4Address("10.1.2.2");
            nextHop[1] = Ipv4Address("10.1.3.2");
            Ipv4StaticRoutingHelper ipv4RoutingHelper;
            Ptr<Ipv4StaticRouting> s1 = ipv4RoutingHelper.GetStaticRouting(i1);
            s1->AddFractionalHostRouteTo(Ipv4Address("10.1.4.2"), nextHop, interfaces, fraction, 2);
        }
}
  int 
main (int argc, char *argv[])
{
  ...
  d1d2Secondary = wifi.Install (wifiPhy, wifiMac, n1n2);
  ...
  pingHelper.SetAttribute("Size", UintegerValue(1000));
