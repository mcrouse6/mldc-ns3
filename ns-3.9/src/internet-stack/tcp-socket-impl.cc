...
TcpSocketImpl::Bind (const Address &address)
{
  ...
  if (ipv4 == Ipv4Address::GetAny () && port == 0)
    {
      m_endPoint = m_tcp->Allocate ();
      NS_LOG_LOGIC ("TcpSocketImpl "<<this<<" got an endpoint 1: "<<m_endPoint);
    }
  else if (ipv4 == Ipv4Address::GetAny () && port != 0)
    {
      m_endPoint = m_tcp->Allocate (port);
      NS_LOG_LOGIC ("TcpSocketImpl "<<this<<" got an endpoint 2: "<<m_endPoint);
    }
  else if (ipv4 != Ipv4Address::GetAny () && port == 0)
    {
      m_endPoint = m_tcp->Allocate (ipv4);
      NS_LOG_LOGIC ("TcpSocketImpl "<<this<<" got an endpoint 3: "<<m_endPoint << " " << ipv4);
    }
  else if (ipv4 != Ipv4Address::GetAny () && port != 0)
    {
      m_endPoint = m_tcp->Allocate (ipv4, port);
      NS_LOG_LOGIC ("TcpSocketImpl "<<this<<" got an endpoint 4: "<<m_endPoint);
    }
...
{
  //std::cout << "========> closing socket " << m_endPoint << std::endl;
  NS_LOG_FUNCTION_NOARGS ();
      ...
      if (route != 0)
        {
          ...
          {
                NS_LOG_LOGIC ("Setting endpoint local address" << route->GetSource ());
                m_endPoint->SetLocalAddress (route->GetSource ());
          }
          else 
          {
                NS_LOG_LOGIC ("NOT Setting endpoint local address because we already have one: " << m_endPoint->GetLocalAddress());
          }
        }
      else
        {
...
Actions_t TcpSocketImpl::ProcessEvent (Events_t e)
{
  ...
  //std::cout << "m_State:" << m_state << " " << saveState << " " << m_endPoint << std::endl;
  //if ((m_state == CLOSED || m_state == TIMED_WAIT) && saveState != CLOSED && m_endPoint != 0)
  if (false && (m_state == CLOSED) && saveState != CLOSED && m_endPoint != 0)
    {
      NS_ASSERT (m_tcp != 0);
      /*
      ...
      //std::cout << "deallocating " << m_endPoint << " " << m_state << std::endl;   
      m_tcp->DeAllocate (m_endPoint);
      m_endPoint = 0;
      CancelAllTimers();
      //std::cout << "done deallocating" << m_endPoint << " " << m_state << std::endl;   
    }
  else if ((m_state == TIMED_WAIT || m_state == CLOSED) && saveState != CLOSED && m_endPoint != 0) {
      m_endPoint->SetClosed(true);
    }
    
  //std::cout << "returning action" << m_endPoint << " " << stateAction.action << std::endl;   
