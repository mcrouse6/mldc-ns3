...
double InterferenceHelper::SlowdownFactor = 1;
...
uint32_t
InterferenceHelper::GetPlcpHeaderDurationNanoSeconds (WifiMode payloadMode, WifiPreamble preamble)
{
  switch (payloadMode.GetModulationClass ())
    {
    case WIFI_MOD_CLASS_OFDM:
    ...
    case WIFI_MOD_CLASS_VHT_SC:
       /* From Table 67 in VHT spec 21.3.4 */
       return 582;
...
uint32_t 
InterferenceHelper::GetPlcpPreambleDurationNanoSeconds (WifiMode payloadMode, WifiPreamble preamble)
{
  switch (payloadMode.GetModulationClass ())
    {
    case WIFI_MOD_CLASS_OFDM:
    ...
    case WIFI_MOD_CLASS_VHT_SC:
    case WIFI_MOD_CLASS_VHT_OFDM:
       /* From Figure 133 and Table 67 in VHT spec */
       return 1745;
...
uint32_t 
InterferenceHelper::GetPayloadDurationNanoSeconds (uint32_t size, WifiMode payloadMode)
{
  ...
  switch (payloadMode.GetModulationClass ())
    ...
    case WIFI_MOD_CLASS_OFDM:
      {
        ...
        case 20000000:
        default:
          symbolDurationNs = 4000;
          break;
        case 10000000:
          symbolDurationNs = 8000;
          break;
        case 5000000:
          symbolDurationNs = 16000;
          break;
        }

        // IEEE Std 802.11-2007, section 17.3.2.2, table 17-3
        // corresponds to N_{DBPS} in the table
        double numDataBitsPerSymbol = payloadMode.GetDataRate ()  * symbolDurationNs / 1e9;

        // IEEE Std 802.11-2007, section 17.3.5.3, equation (17-11)
        uint32_t numSymbols = lrint (ceil ((16 + size * 8.0 + 6.0)/numDataBitsPerSymbol));

        return lrint(ceil (numSymbols*symbolDurationNs*SlowdownFactor));
      }
    ...
    case WIFI_MOD_CLASS_DSSS:
      ...
      return lrint(ceil ((size * 8.0) / payloadMode.GetDataRate () * 1.0e9*SlowdownFactor));

    case WIFI_MOD_CLASS_VHT_SC:
      {
        /* 21.3.4, Table 67 = Nblks * 512 * T_c */
        /* 21.6.3.2.2.2 (5), Compute Nblks: */
        uint32_t Ncbpb;
        ...
        uint32_t Ncw = (uint32_t)ceil(Ncbits / 672.0);
        uint32_t Nblks = (uint32_t)ceil(Ncw * 672.0 / Ncbpb);
        /* Make sure the result is in Nanoseconds */
        double ret = Nblks * 1.0e9 * 512 / 1760.0e6;
        NS_LOG_DEBUG("bits " << Nbits << " cbits " << Ncbits << " rate " << payloadMode.GetDataRate() << " time " << ret << " ns");
        return lrint(ceil(ret*SlowdownFactor));
      }

    case WIFI_MOD_CLASS_VHT_OFDM:
      {
        /* 21.3.4, Table 67 = Nblks * 512 * T_c */
        /* 21.5.3.2.2.2 (5), Compute Nblks: */
        uint32_t Ncbps;
        if (payloadMode.GetConstellationSize() == 2)
                Ncbps = 336;
        else if (payloadMode.GetConstellationSize() == 4)
        ...
        if (payloadMode.GetCodeRate() == WIFI_CODE_RATE_1_4)
                ...
                NS_FATAL_ERROR("unsupported code rate");
        uint32_t Ncw = (uint32_t)ceil(Ncbits / 672.0);
        uint32_t Nsym = (uint32_t)ceil(Ncw * 672.0 / Ncbps);
        /* Make sure the result is in Nanoseconds */
        double ret = Nsym * 242;
        NS_LOG_DEBUG("bits " << Nbits << " cbits " << Ncbits << " rate " << payloadMode.GetDataRate() << " time " << ret << " ns");
        return lrint(ceil(ret*SlowdownFactor));
...
{
  uint32_t duration = GetPlcpPreambleDurationNanoSeconds (payloadMode, preamble)     
                      + GetPlcpHeaderDurationNanoSeconds (payloadMode, preamble)  
                      + GetPayloadDurationNanoSeconds (size, payloadMode);
  ...
  if (!m_rxing)
    {
      ...
      if (m_firstPower < 0)
        m_firstPower = 0;        /* DAN: this should be true, and fixed double precision errors */
...
InterferenceHelper::CalculateChunkSuccessRate (double snir, Time duration, WifiMode mode) const
{
  ...
  uint64_t rate = mode.GetPhyRate ();
  uint64_t nbits = rate * duration.GetSeconds ();
...
{  
  ...
  Time plcpHeaderStart = (*j).GetTime () + NanoSeconds (GetPlcpPreambleDurationNanoSeconds (payloadMode, preamble));
  Time plcpPayloadStart = plcpHeaderStart + NanoSeconds (GetPlcpHeaderDurationNanoSeconds (payloadMode, preamble));
