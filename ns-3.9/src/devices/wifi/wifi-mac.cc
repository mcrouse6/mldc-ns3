...
void
WifiMac::Configure80211ad_SC (void)
{
  SetSifs(MicroSeconds(3));	/* aSIFSTime in TGad Draft 0.1 Table 90 */
  ...
  /* 581.818 ns for ACK using MCS1, plus preamble and PLCP header */
  SetEifsNoDifs(GetSifs() + NanoSeconds(2909));
  SetCtsTimeout(GetSifs() + NanoSeconds(2909) + GetSlot() +
		  GetMaxPropagationDelay() + GetMaxPropagationDelay());
  SetAckTimeout(GetSifs() + NanoSeconds(2909) + GetSlot() +
		  GetMaxPropagationDelay() + GetMaxPropagationDelay());
}
...
WifiMac::Configure80211ad_OFDM (void)
{
  ...
  SetCtsTimeout(GetSifs() + NanoSeconds(2229) + GetSlot() +
		  GetMaxPropagationDelay() + GetMaxPropagationDelay());
  SetAckTimeout(GetSifs() + NanoSeconds(2229) + GetSlot() +
		  GetMaxPropagationDelay() + GetMaxPropagationDelay());
}
