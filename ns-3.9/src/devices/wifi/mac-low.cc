...
MacLow::CtsTimeout (void)
{
  ...
  m_listener = 0;
  if (!listener)
	  NS_FATAL_ERROR("listener is null");
  listener->MissedCts ();
}
void
...
MacLow::StartDataTxTimers (void)
{
  ...
  if (m_txParams.MustWaitNormalAck ()) 
  ...
  else 
    {
      ...
      NS_LOG_DEBUG(this << "setting m_listener to NULL");
      m_listener = 0;
    }
}
...
MacLow::SendDataPacket (void)
{
  ...
  NS_LOG_DEBUG("duration(post-this-payload)=" << duration);
...
MacLow::SendAckAfterData (Mac48Address source, Time duration, WifiMode dataTxMode, double dataSnr)
{
  ...
  //NS_LOG_UNCOND("ACKDuration " <<  GetAckDuration (source, dataTxMode) << " duration " << duration);
  duration -= GetSifs ();
  //NS_LOG_UNCOND("SIFS " <<  GetSifs() << " duration " << duration);
  NS_ASSERT (duration >= MicroSeconds (0));
