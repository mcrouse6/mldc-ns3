...
void OnOffApplication::StopApplication() // Called at time specified by Stop
{
  ...
  if(m_socket != 0)
    {
      //std::cout << "Here 2:" << __func__ << std::endl;
      m_socket->Close ();
  ...
  if (m_maxBytes == 0 || m_totBytes < m_maxBytes)
    {
      ...
      if (m_maxBytes > 0 && bits/8 > (m_maxBytes -  m_totBytes))
          bits = (m_maxBytes - m_totBytes)*8;
      if (bits <= 0)
          StopApplication();
      ...
      m_sendEvent = Simulator::Schedule(nextTime, 
                                        &OnOffApplication::SendPacket, this, bits/8);
    }
  else
...
void OnOffApplication::SendPacket(uint32_t size)
{
  NS_LOG_FUNCTION_NOARGS ();
